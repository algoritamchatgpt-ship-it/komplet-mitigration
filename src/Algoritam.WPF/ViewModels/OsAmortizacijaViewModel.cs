...
public partial class OsAmortizacijaViewModel : ObservableObject
{
    private readonly string _folderPath;
    private List<Dictionary<string, object?>> _sveStavke = [];

    public string Naslov => "OBRAČUN AMORTIZACIJE OSNOVNIH SREDSTAVA";
    ...
    public decimal UkupnoAmort => Stavke.Sum(s => ExtractDecimal(s, "AMORT"));
    public decimal UkupnoNbv   => Stavke.Sum(s => ExtractDecimal(s, "NBV"));

    public event Action? ZatvaranjeZahtevano;

    public OsAmortizacijaViewModel(string folderPath)
    {
    ...
    private async Task UcitajAsync()
    {
        ...
        try
        {
            var dbfPath = NadjiDbf(_folderPath, "osamort.dbf");
            if (dbfPath is null)
            {
                StatusPoruka = "Tabela osamort.dbf nije pronađena.";
                return;
            }
    ...
    private void PrimenjiFilter()
    {
        ...
        OnPropertyChanged(nameof(UkupnoAmort));
        OnPropertyChanged(nameof(UkupnoNbv));
    }
