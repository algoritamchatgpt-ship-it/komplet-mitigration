...
/// <summary>
/// Pregled analitičke kartice partnera — ana.dbf sa filterom po kontu i partneru.
/// </summary>
public partial class AnKarticaViewModel : ObservableObject
{
    ...
    public AnKarticaViewModel(string folderPath, string dbfName = "ana.dbf",
        string naslov = "KARTICA PARTNERA — ANALITIKA")
    {
    ...
    private void PrimenjiFilter()
    {
        ...
        if (!string.IsNullOrEmpty(konto))
            filtrirane = filtrirane.Where(s =>
                (s.TryGetValue("KONTO",   out var k) && k?.ToString()?.ToLowerInvariant().StartsWith(konto) == true) ||
                (s.TryGetValue("NALOG",   out var n) && n?.ToString()?.ToLowerInvariant().Contains(konto)  == true));

        if (!string.IsNullOrEmpty(partner))
            filtrirane = filtrirane.Where(s =>
                s.Values.Any(v => v?.ToString()?.ToLowerInvariant().Contains(partner) == true));
