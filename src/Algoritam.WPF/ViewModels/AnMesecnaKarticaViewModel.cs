...
public partial class AnMesecnaKarticaViewModel : ObservableObject
{
    ...
    [ObservableProperty] private string _filterGodina = "";

    private List<Dictionary<string, object?>> _sveStavke = [];

    public event Action? ZatvaranjeZahtevano;

    public AnMesecnaKarticaViewModel(string folderPath,
        string dbfName = "anmes.dbf",
        string naslov = "KARTICA PARTNERA PO MESECIMA")
    {
    ...
    partial void OnFilterGodinaChanged(string value)  => PrimenjiFilter();

    private void PrimenjiFilter()
    {
        var partner = FilterPartner.Trim().ToLowerInvariant();
        var godina  = FilterGodina.Trim();
        ...
        if (!string.IsNullOrEmpty(godina))
            filtrirane = filtrirane.Where(s =>
                s.TryGetValue("GODINA", out var g) && g?.ToString()?.StartsWith(godina) == true);
